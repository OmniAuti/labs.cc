...
function decodeBatchID($batchID) {
	...
	if(str_contains(strtoupper($batchID), "XD")  || str_contains(strtoupper($batchID), "MFG")) {
  ...
  if(preg_match_all('/[0-9]{8}/', $batchID) > 1 && $is_concentrate === false) {
	...
	if($is_mfg) {
		...
		// Pull everything from XD: onward, so we can check if XD has hyphenated date
		$xdSub = substr($batchID, strpos($batchID, "MFG"));
		// If XD has hyphenated date, pull different substring for manufacture date
		if(str_contains( $xdSub, "-" )) {
			$unformattedXD = substr($xdSub, 3, 8);
		} else { 
	...
	return array($formattedHD, $formattedXD, $is_concentrate, $is_mfg);
}
...
<main class="main">
		...
		<p><?= $batch_id1 ?></p>
		<p>HD: <?= $batch_testing[0]; ?></p>
		<p>MFG: <?= $batch_testing[1]; ?></p>
		<p>Is MFG:<?= $batch_testing[3]; ?></p>
</main>
