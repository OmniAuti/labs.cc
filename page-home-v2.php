...
$user_role = $user->roles[0];
	...
	// check if there is a "batch" post with a title that matches the lab ID
	// $batch_post_exists = $wpdb->get_var( 
	// 	"SELECT COUNT(*) FROM {$wpdb->prefix}lab_data WHERE lab_id = '$lab_id'",
	//  );

	// $batch_post_exists = $wpdb->get_results( $wpdb->prepare("SELECT * FROM $wpdb->posts WHERE post_title LIKE '%s'", '%'. $wpdb->esc_like( $lab_id ) .'%') );
	$batch_post_exists = $wpdb->get_results( 
		$wpdb->prepare( 
	...
	$exists = $wpdb->get_var( 
	 ...
	 if ($batch_post_exists && $lab_id != "") {
		wp_redirect( "\/b/" . $lab_id, $status = 301, $x_redirect_by = 'WordPress' );

	} elseif ( ! $exists && $lab_id != "" ) {
		$data = ccApiCall($lab_id, $key);
		// $lab_id = checkIDCase();
		$batch_id = preg_replace('/^\s*/','', $data['sample']['batch_id']);
		...
		$lab_id = $data->lab_id;
		$batch_id = preg_replace('/^\s*/','', $data->batch_id);
	...
	function decodeBatchID($batchID) {
		// Clean up $batchID
		$clean_batchID = str_replace(' ', '', $batchID);
		// Check if batch is concentrate
		if(str_contains(strtoupper($clean_batchID), "XD")) {
			$is_concentrate = true;
			$is_mfg = false;
		} elseif(str_contains(strtoupper($clean_batchID), "MFG")) {
			$is_mfg = true;
			$is_concentrate = true;
		} else {
		...
		if(preg_match_all('/[0-9]{8}/', $clean_batchID) > 1 && $is_concentrate == false) {
			preg_match_all('/[0-9]{8}/', $clean_batchID, $match);
		...
		} elseif(preg_match_all('/[0-9]{8}/', $clean_batchID) > 1 && $is_mfg == true) {
			preg_match_all('/[0-9]{8}/', $clean_batchID, $match);
			$unformattedHD0 = substr($match[0][0], 0, 8);
			$yearHD0 = substr($unformattedHD0, 0, 4);
			$monthHD0 = substr($unformattedHD0, 4, 2);
			...
			$unformattedHD = substr($clean_batchID, 0, 8);
...
<main class="main">
	...
	?>
		<form action="/delete-confirmation/" method="POST">
			<input type="hidden" value="<?= $lab_id ?>" name="lab_id">
			<input type="hidden" value="<?= $refresh_url ?>" name="refresh_url">
			<button id="admin--delete-record-btn" class="brand-button" type="submit">Refresh This Record</button>
		</form>
	<?php
		endif;
	?>
	<section class="qrlab-info">
		<div class="information-container">
			...
			<?php if($manufacture_date) { ?>
				<div class="cultivation-container">
				<p>Manufactured by Greens Goddess Products, Inc.</p>
				<p>Establishment License Number: <span>00000106DCQV00747138</span></p>
			</div>
			<?php } ?>
