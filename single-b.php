<?php 
/* 
Template Name: Extract / Flower Batches
...
$user_role = $user->roles[0];
    ...
    $lab_id = strtoupper(str_replace("-", ".", basename($link)));
		// if(!empty($_GET['ID'])) {
		// 	$lab_id = $_GET['ID'];
...
<main class="main">
	...
	<section class="qrlab-info">
      ...
      <div class="flower-lots">
				<?php
          if( have_rows('parent_flower_batches') ): 
					?>
						<h3 style="margin-bottom:0">Source Material:</h3>
						<?php
              while( have_rows('parent_flower_batches') ) : the_row();
                  $strain = get_sub_field('strain');
                  // $harvest_date = get_sub_field('harvest_date');
                  $batch_number = get_sub_field('batch_number');
                  $lab_id = get_sub_field('lab_id');
                  ?>
									<div class="lot">
                    <p>Strain: <?= $strain ?></p>
                    <p>Harvest Date: <?php 
											echo decodeBatchID($batch_number)[0];
										 ?></p>
